...
export function TaskCard({ todo, onToggle, onDelete, onEdit }) {
  ...
  return (
    <Collapsible
      ...
      className={cn(
        'w-full border rounded-xl bg-card transition-all duration-300 ease-in-out',
        isOpen ? 'border-primary/40 shadow-lg bg-accent/5' : 'border-border/40 hover:border-border/80 hover:shadow-sm',
      ...
      <div className="flex items-center p-4 gap-4">
        ...
        <div
          className={cn(
            ...
            todo.completed ? 'h-6 opacity-20' : 'h-8 opacity-60'
          )}
        />
        ...
        {/* Task Title  */}
        <div className="flex-1 cursor-pointer select-none" onClick={() => setIsOpen(!isOpen)}>
          <span
            className={cn(
      ...
      <CollapsibleContent className="overflow-hidden data-[state=open]:animate-collapsible-down data-[state=closed]:animate-collapsible-up">
        <div className="px-6 pb-5 pt-0 space-y-5 transition-opacity duration-500 data-[state=closed]:opacity-0 data-[state=open]:opacity-100">
          ...
          <div className="flex items-center justify-end gap-2 pt-3 border-t border-border/40">
            ...
            <DeleteTaskDialog onConfirm={() => onDelete(todo.id)}>
              <Button
                variant="ghost"
                size="sm"
